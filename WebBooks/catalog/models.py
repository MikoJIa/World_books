...
class Author(models.Model):
    ...
    photo = models.ImageField(upload_to='images', help_text='Введите фото автора',
...
class Book(models.Model):
    ...
    def get_absolute_url(self):
        # Возвращает URL-адрес для доступа
        # к определенному экземпляру книги
        return reverse('book-detail', args=[str(self.id)])
    #К сожалению, . мы не можем напрямую показать поле author, т. к. между авторами и
    #книгами имеется связь «многие ко многим» (мanyТoManyFielct), поскольку у одной книги
    #может быть несколько авторов.
    # Для того чтобы сформировать список авторов книги,
    # мы добавим функцию (метод) display_author в модели данных вооk.

    def display_author(self):
        return ', '.join([author.last_name for author in self.author.all()])

    # В теле цикла выбираются все
    # авторы, связанные с данной книгой, из них формируется список, который будет
    # возвращен в точку вызова. Для этого списка формируется короткое имя «Авторы»:
...
class BookInstance(models.Model):
    ...
    status = models.ForeignKey(Status, on_delete=models.CASCADE, null=True,
                               help_text='Изменить статус экземпляра',
                               verbose_name='Статус экземпляра книги')
